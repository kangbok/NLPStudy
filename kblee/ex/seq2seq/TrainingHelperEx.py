##### https://github.com/j-min/tf_tutorial_plus/blob/master/RNN_seq2seq/contrib_seq2seq/01_TrainingHelper.ipynb
##### code from above URL.

import matplotlib.pyplot as plt # To plot learning curve grap
from pprint import pprint # for pretty print
import re # for tokenizer
from collections import Counter # for word counter in vocabulary dictionary
import os # for checkpoint paths
from tqdm import tqdm # for fancy progress bar
...
enc_vocab, enc_reverse_vocab, enc_vocab_size = build_vocab(all_input_sentences, is_target=False)
...
class Seq2SeqModel(object):
    ...
    def add_decoder(self):
        with tf.variable_scope('Decoder') as scope:
            ...
            output_layer = tf.layers.Dense(dec_vocab_size + 2, name='output_projection')
            ...
            if self.mode == 'training':
                ...
                training_helper = contrib.seq2seq.TrainingHelper(
                    inputs=dec_emb_inputs,
                    sequence_length=self.dec_sequence_length + 1,
                    time_major=False,
                    name='training_helper')

                training_decoder = contrib.seq2seq.BasicDecoder(
                    cell=dec_cell,
                    helper=training_helper,
                    initial_state=self.enc_last_state,
                    output_layer=output_layer)

                train_dec_outputs, train_dec_last_state, _ = contrib.seq2seq.dynamic_decode(
                    training_decoder,
                ...
                self.batch_loss = contrib.seq2seq.sequence_loss(
                    logits=logits,
                    targets=targets,
                    weights=masks,
                    name='batch_loss')
                ...
                inference_helper = contrib.seq2seq.GreedyEmbeddingHelper(
                    embedding=self.dec_Wemb,
                    start_tokens=start_tokens,
                    end_token=1)

                inference_decoder = contrib.seq2seq.BasicDecoder(
                    cell=dec_cell,
                    helper=inference_helper,
                    initial_state=self.enc_last_state,
                    output_layer=output_layer)

                infer_dec_outputs, infer_dec_last_state, _ = contrib.seq2seq.dynamic_decode(
                    inference_decoder,
                    output_time_major=False,
                    impute_finished=True,
